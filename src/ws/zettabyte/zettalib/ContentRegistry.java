...
public class ContentRegistry implements IFuelHandler
{
    ...
    private HashMap<ImmutablePair<Integer, Integer>, Integer> fuelDataSpecific = new HashMap<ImmutablePair<Integer, Integer>, Integer>(10);
    //For Vanilla furnaces. Item ID to burn time, does not care about damage value.
    private HashMap<Integer, Integer> fuelDataSimple = new HashMap<Integer, Integer>(10);
            ...
            if (item instanceof IWeirdScienceItem)
            {
                ...
                if (subItems != null)
                {
                    for (ISubItem si : subItems)
                    {
                        ...
                        {
                            fuelDataSpecific.put(new ImmutablePair<Integer, Integer>(item.itemID, si.getAssociatedMeta()), si.getFurnaceFuelValue());
                        }
                    }
                }
