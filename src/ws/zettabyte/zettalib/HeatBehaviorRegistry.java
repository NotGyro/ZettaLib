...
public class HeatBehaviorRegistry
{
     ...
     */
    private ArrayList<ArrayList<BlockHeatBehavior>> blockBehavior = new ArrayList<ArrayList<BlockHeatBehavior>>(4096);

    //Temperature is in degrees celsius.
    public int TryBlockBehaviorAt (World world, int x, int y, int z, int temperature)
    {
        ArrayList<BlockHeatBehavior> listbhb = blockBehavior.get(world.getBlockId(x, y, z));
        if (listbhb != null)
        {
    ...
    {
        if (blockBehavior.get(block.blockID) == null)
        {
            blockBehavior.add(block.blockID, (new ArrayList<BlockHeatBehavior>(2)));
        }

        blockBehavior.get(block.blockID).add(bhb);
    }

    //Mapping of entity ID to heat behavior list.
    HashMap<Integer, ArrayList<EntityHeatBehavior>> entityBehavior = new HashMap<Integer, ArrayList<EntityHeatBehavior>>(8);

    //Temperature is in degrees celsius.
    public int TryEntityBehavior (Entity ent, int temperature)
    {
        ArrayList<EntityHeatBehavior> listbehv = entityBehavior.get(ent.entityId);
        if (listbehv != null)
        {
    ...
    public void AddEntityBehavior (int entityId, EntityHeatBehavior behv)
    {
        if (entityBehavior.get(entityId) == null)
        {
            entityBehavior.put(entityId, (new ArrayList<EntityHeatBehavior>(1)));
        }

        entityBehavior.get(entityId).add(behv);
    }
}
